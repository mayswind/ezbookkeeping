...
func (s *TransactionCategoryService) ModifyCategory(c core.Context, category *models.TransactionCategory) error {
	...
	return s.UserDataDB(category.Uid).DoTransaction(c, func(sess *xorm.Session) error {
		updatedRows, err := sess.ID(category.CategoryId).Cols("parent_category_id", "name", "display_order", "icon", "icon_type", "color", "comment", "budget_amount", "budget_currency", "hidden", "updated_unix_time").Where("uid=? AND deleted=?", category.Uid, false).Update(category)
